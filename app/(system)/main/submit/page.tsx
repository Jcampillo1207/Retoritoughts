...
export default function SubmitPage() {
  return (
    <main className="w-full h-fit min-h-[calc(100dvh_-_56px)] lg:h-[calc(100dvh_-_56px)] lg:max-h-[100dvh] items-start justify-start flex flex-col lg:flex-row sticky top-0 overflow-hidden">
      <aside className="w-full h-fit lg:h-full border-b lg:border-b-0 lg:border-r col-span-1 lg:col-span-2 px-5 md:px-7 py-10 lg:max-w-xs">
        <Button
          variant={"ghost"}
          size={"sm"}
          asChild
          className="flex items-center justify-normal gap-x-2 text-muted-foreground w-fit"
        >
          <Link href={"/main"}>
      ...
      <div className="w-full flex-1 flex flex-col gap-y-10 h-full overflow-scroll px-5 md:px-7 py-10 bg-muted dark:bg-muted/10 items-start justify-start">
