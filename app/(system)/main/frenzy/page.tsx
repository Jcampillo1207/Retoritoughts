...
import { getFrenzyHighScore, getHighScore, updateFrenzyScore, updateScore } from "@/lib/supabase/score";
...
import { AlertDialogComponent } from "../play/_components/alertDialog";
...
export default function FrenzyPage() {
  ...
  useEffect(() => {
    ...
    const updateProgressDecrement = () => {
      const decrement = 1;

      // Calcula el intervalo dinámico basado en el score.
      // A medida que el score aumenta, el intervalo disminuye, pero no menos de 50ms.
      const dynamicInterval = score < 999 ? Math.max(100 - score, 50) : 50;
      if (isActive && progress > 0) {
        interval = setInterval(() => {
          setProgress((prevProgress) => {
            if (prevProgress > 0) {
              const nextProgress = prevProgress - decrement;
              if (nextProgress <= 0) {
                setLose(true);
    ...
    updateProgressDecrement();

    return () => {
      if (interval) clearInterval(interval);
    };
  }, [isActive, progress, score]);
    ...
    const fetchUser = async () => {
      try {
        ...
        if (error) {
          ...
          if (userData) {
            setHighscore(await getFrenzyHighScore(userData.email));
          }
        }
      } catch (error) {
  ...
  async function checkAnswerSelected() {
    ...
    if (event === undefined) {
      ...
      if (result) {
        ...
        const newHighScore = await updateFrenzyScore(score, userData.email);
        setScore(0);
        setLose(true);
        // Loss
      }
                  ...
                  return (
                    ...
                    >
                      ...
                      {item.decription && (
                        <>
                          <Separator />
                          <p className="tracking-wide text-muted-foreground text-xs lg:text-base line-clamp-6">
                            {item.decription}
                          </p>
                        </>
                      )}
