...
import {
  getHighScore,
  getRealisticHighScore,
  updateRealScore,
  updateScore,
} from "@/lib/supabase/score";
...
export default function RealisticPage() {
    ...
    const fetchUser = async () => {
      try {
        ...
        if (error) {
          ...
          if (userData) {
            setHighscore(await getRealisticHighScore(userData.email));
          }
        }
      } catch (error) {
  ...
  useEffect(() => {
    ...
    async function fetch() {
      const data = await fetchRealData(skeletonCount);
      setInfo(data);
      setImages(data);
      setIsLoading(false);
      setIsActive(true);
  ...
  async function checkAnswerSelected() {
    ...
    if (event === undefined) {
      ...
      if (result) {
        ...
        const newHighScore = await updateRealScore(score, userData.email);
        setScore(0);
        setLose(true);
        // Loss
      }
        ...
        <div className="w-full h-full z-[9999] bg-muted/50 backdrop-blur-md flex items-center justify-center fixed top-0 left-0 px-5 md:px-7 lg:px-14">
          <div className="w-full max-w-4xl bg-background border p-5 md:p-7 rounded-2xl h-fit flex flex-col gap-y-7 lg:gap-y-7">
            <h1 className="text-lg font-bold md:text-xl lg:text-2xl max-w-sm text-pretty mb-5 lg:mb-7">
              Oops, loks like you need to study more{" "}
